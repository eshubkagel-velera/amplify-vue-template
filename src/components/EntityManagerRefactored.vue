  ...
  <div class="entity-manager">
    ...
    <div class="entity-list">
      ...
      <DataTable
        ...
        :fields="comparisonMode ? fields : filteredFields"
        :filters="filters"
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isUpdate) {
      ...
      successMessage.value = processedData.shouldCreateCopy 
        ? `New ${props.entityName} created (original has mappings)!`
        : `${props.entityName} updated successfully!`;
    } else {
...
const applyFilters = () => {
  let filtered = entities.value.filter(entity => {
    return filteredFields.value.every(field => {
      const filterValue = filters.value[field];
      if (!filterValue) return true;
      const entityValue = String(entity[field] || '').toLowerCase();
      return entityValue.includes(filterValue.toLowerCase());
