  ...
  <div class="entity-manager">
        ...
        <table class="entity-table">
          ...
          <tbody>
            <tr v-for="entity in filteredEntities" :key="getEntityId(entity)">
              ...
              <td>
                <button @click="editEntity(entity)" :aria-label="`Edit ${entityName} ${getEntityId(entity)}`">Edit</button>
    ...
    <div v-if="showEditModal" class="modal-overlay">
      <div class="modal-content">
        ...
        <form @submit.prevent="submitForm">
          <div v-for="field in formFields" :key="field.name" class="form-group" v-show="!((props.entityName === 'ORIGIN_PRODUCT' || props.entityName === 'REDIRECT_URL' || props.entityName === 'SERVICE' || props.entityName === 'SERVICE_PARAM' || props.entityName === 'SERVICE_PARAM_MAPPING' || props.entityName === 'SERVICE_EXPR_MAPPING' || props.entityName === 'SERVICE_PROVIDER' || props.entityName === 'STEP_TYPE') && (field.name === 'CREATED_DATE' || field.name === 'CREATED_BY_USER_ID'))">
            <label :for="field.name">{{ field.name }}</label>
            <div v-if="field.name === 'VENDOR_NAME' && props.entityName === 'ORIGIN_PRODUCT'">
              <input 
                :id="field.name" 
          ...
          <div v-if="props.entityName === 'ORIGIN_PRODUCT' || props.entityName === 'REDIRECT_URL' || props.entityName === 'SERVICE_PROVIDER' || props.entityName === 'STEP_TYPE' || props.entityName === 'SERVICE_PARAM' || props.entityName === 'SERVICE'" class="form-group">
...
const { selectedItems: selectedEntities, allSelected, toggleSelectAll: toggleSelectAllItems, sortBy: sortByField, applyFilters: applyTableFilters } = useTableOperations(entities, getEntityId);
...
const editEntity = (entity) => {
  ...
  if (props.entityName === 'ORIGIN_PRODUCT' || props.entityName === 'REDIRECT_URL' || props.entityName === 'SERVICE_PROVIDER' || props.entityName === 'STEP_TYPE' || props.entityName === 'SERVICE_PARAM' || props.entityName === 'SERVICE') {
    const today = new Date();
    const year = today.getFullYear();
    const month = String(today.getMonth() + 1).padStart(2, '0');
    const day = String(today.getDate()).padStart(2, '0');
    formattedEntity.CHANGED_DATE = `${year}-${month}-${day}`;
...
watch(showCreateModal, (newVal) => {
  if (newVal) {
    ...
    if (props.entityName === 'ORIGIN_PRODUCT' || props.entityName === 'REDIRECT_URL' || props.entityName === 'SERVICE' || props.entityName === 'SERVICE_PARAM' || props.entityName === 'SERVICE_PARAM_MAPPING' || props.entityName === 'SERVICE_EXPR_MAPPING' || props.entityName === 'SERVICE_PROVIDER' || props.entityName === 'STEP_TYPE') {
...
const filterByStepType = async () => {
  ...
  if (selectedStepFilter.value) {
    try {
      const [mappingsResponse, stepTypesResponse, servicesResponse] = await Promise.all([
        fetch('https://fi5pjed64nf4ran34tusrlvi7u.appsync-api.us-east-2.amazonaws.com/graphql', {
          ...
          body: JSON.stringify({
            query: `
              query ListStepServiceMappings($filter: TableSTEP_SERVICE_MAPPINGFilterInput) {
                listSTEP_SERVICE_MAPPINGS(filter: $filter, limit: 1000) {
                  items {
...
const enhanceServiceParams = async (serviceParams) => {
  if (props.entityName !== 'SERVICE_PARAM') return serviceParams;
