import { sql, createMySQLStatement } from '@aws-appsync/utils/rds';

export function request(ctx) {
  const { STEP_TYPE_PARAM_MAP_ID } = ctx.args.input;
  
  const preimage = sql`
    SELECT * FROM STEP_TYPE_PARAM_MAP
    WHERE STEP_TYPE_PARAM_MAP_ID = ${STEP_TYPE_PARAM_MAP_ID}
    LIMIT 1
  `;
  
  const del = sql`DELETE FROM STEP_TYPE_PARAM_MAP WHERE STEP_TYPE_PARAM_MAP_ID = ${STEP_TYPE_PARAM_MAP_ID}`;
  
  return createMySQLStatement(preimage, del);
}

export function response(ctx) {
  const pre = ctx.result?.[0] ?? [];
  return pre[0] ?? null;
}