...
export function request(ctx) {
  ...
  Object.keys(i).forEach(key => {
    if (key !== 'SERVICE_PARAM_MAPPING_ID' && i[key] !== undefined) {
  ...
  const update = sql`
    ...
    WHERE SERVICE_PARAM_MAPPING_ID = ${i.SERVICE_PARAM_MAPPING_ID}
  `;
  
  const select = sql`
    SELECT * FROM SERVICE_PARAM_MAPPING WHERE SERVICE_PARAM_MAPPING_ID = ${i.SERVICE_PARAM_MAPPING_ID}
  `;
