...
export function request(ctx) {
  ...
  Object.keys(i).forEach(key => {
    if (key !== 'SERVICE_DOMAIN_ID' && i[key] !== undefined) {
  ...
  const update = sql`
    ...
    WHERE SERVICE_DOMAIN_ID = ${i.SERVICE_DOMAIN_ID}
  `;
  
  const select = sql`
    SELECT * FROM SERVICE_DOMAIN WHERE SERVICE_DOMAIN_ID = ${i.SERVICE_DOMAIN_ID}
  `;
